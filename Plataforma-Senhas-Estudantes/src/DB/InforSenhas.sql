...
DROP TABLE IF EXISTS Estado_Senha;
DROP TABLE IF EXISTS Estado_Cartao;
...
CREATE TABLE Estado_Cartao (
    Id_Estado_Cartao INT AUTO_INCREMENT PRIMARY KEY,
    Estado VARCHAR(50) NOT NULL,
    UNIQUE (Estado)
);

CREATE TABLE Estado_Senha (
    Id_Estado_Senha INT AUTO_INCREMENT PRIMARY KEY,
    Estado VARCHAR(50) NOT NULL,
    UNIQUE (Estado)
);
...
ALTER TABLE Auxiliar ADD CONSTRAINT fk_auxiliar_turno FOREIGN KEY (Turno) REFERENCES Turno(Id_Turno);
...
ALTER TABLE Cartao ADD CONSTRAINT fk_cartao_aluno FOREIGN KEY (Aluno) REFERENCES Aluno(Id_Aluno);
ALTER TABLE Cartao ADD CONSTRAINT fk_cartao_estado FOREIGN KEY (Estado_Cartao) REFERENCES Estado_Cartao(Id_Estado_Cartao);
...
ALTER TABLE Compra ADD CONSTRAINT fk_compra_aluno FOREIGN KEY (Aluno) REFERENCES Aluno(Id_Aluno);

CREATE TABLE Senha (
    Id_Senha INT AUTO_INCREMENT PRIMARY KEY,
    Compra INT NOT NULL,
    Estado_Senha INT NOT NULL,
